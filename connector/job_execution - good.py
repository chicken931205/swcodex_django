...
def update_column_metadata(script, job, original_column_names, file_path):
    ...
    try:
        ...
        for orig_name, curr_name in columns_to_update:
            ...
            column.save()
            logger.info(f"Updated column: {column.column_name} (override: {column.override_column_name}, is_unique: {column.is_unique})")
...
def execute_sql_import(script, job):
    ...
    try:
        ...
        if file_path.lower().endswith('.csv'):
            df_sample = pd.read_csv(file_path, nrows=1000, dtype=str, encoding='utf-8-sig', header=0)
            logger.info("Reading CSV file (first 1000 rows for type inference)")
        elif file_path.lower().endswith('.xlsx'):
            df_sample = pd.read_excel(file_path, nrows=1000, dtype=str)
            logger.info("Reading Excel file (first 1000 rows for type inference)")
        elif file_path.lower().endswith('.json'):
            df_sample = pd.read_json(file_path, nrows=1000, dtype=str, encoding='utf-8-sig', header=0)
