...
def read_sample_data(file_path, nrows=100000):
    if file_path.lower().endswith('.csv'):
...
def infer_column_types(df_sample):
    ...
    for col in df_sample.columns:
        if df_sample[col].isnull().all():
            ...
            inferred_types[col] = 'DATE' if is_date_only(df_sample[col]) else 'DATETIME'
        else:
            inferred_types[col] = determine_string_type(df_sample[col])
    return inferred_types

def is_likely_integer(series):
    # Drop NaN values for type checking, but keep track of whether NaNs were present
    had_nulls = series.isnull().any()
    ...
    try:
        # Convert to numeric, allowing int and float
        numeric_series = pd.to_numeric(series, errors='raise')
        
        # Check if all non-null values are integers
        is_integer = (numeric_series.mod(1) == 0).all()
        
        # Check if all values are within the INT range
        in_range = (numeric_series >= -2147483648) & (numeric_series <= 2147483647).all()
        
        # If it had nulls and all non-null values are integers in range, it's likely an integer column
        # If it had no nulls, all values must be integers in range
        return (had_nulls and is_integer and in_range) or (not had_nulls and is_integer and in_range)
    except ValueError:
...
def is_likely_float(series):
    try:
        ...
        return not float_series.isnull().all() and not series.astype(str).str.contains(r'[a-zA-Z]').any() and not is_likely_integer(series)
    except:
        return False
